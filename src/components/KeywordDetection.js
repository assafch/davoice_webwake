import React, { useState, useEffect } from 'react';

const KeywordDetection = () => {
  const [isListening, setIsListening] = useState(false);
  const [transcript, setTranscript] = useState('');
  const [keywordDetected, setKeywordDetected] = useState(false);

  let recognition;

  useEffect(() => {
    // Check if the browser supports the Web Speech API
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      alert('Your browser does not support speech recognition. Please use Chrome or Edge.');
      return;
    }

    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    recognition = new SpeechRecognition();

    recognition.continuous = true;
    recognition.interimResults = true;
    recognition.lang = 'en-US';

    recognition.onstart = () => {
      setIsListening(true);
    };

    recognition.onend = () => {
      setIsListening(false);
    };

    recognition.onresult = (event) => {
      let interimTranscript = '';
      let finalTranscript = '';

      for (let i = event.resultIndex; i < event.results.length; ++i) {
        const transcriptSegment = event.results[i][0].transcript;
        if (event.results[i].isFinal) {
          finalTranscript += transcriptSegment;
        } else {
          interimTranscript += transcriptSegment;
        }
      }

      const combinedTranscript = finalTranscript + interimTranscript;
      setTranscript(combinedTranscript);

      // Define your keyword(s) here
      const keywords = ['carlito'];

      // Check if any keyword is detected in the transcript
      keywords.forEach((keyword) => {
        if (combinedTranscript.toLowerCase().includes(keyword.toLowerCase())) {
          setKeywordDetected(true);
          // Stop recognition if keyword is detected
          recognition.stop();
          // Perform any action you need when keyword is detected
          alert(`Keyword detected: "${keyword}"`);
        }
      });
    };

    recognition.onerror = (event) => {
      console.error('Speech recognition error', event.error);
    };

    // Cleanup on component unmount
    return () => {
      if (recognition && isListening) {
        recognition.stop();
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleStartListening = () => {
    if (recognition && !isListening) {
      recognition.start();
    }
  };

  const handleStopListening = () => {
    if (recognition && isListening) {
      recognition.stop();
    }
  };

  return (
    <div className="keyword-detection-container">
      <button
        className="btn btn-light rounded-circle p-3 shadow"
        onClick={handleStartListening}
        disabled={isListening}
      >
        {/* Microphone SVG Icon */}
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          width="48px"
          height="48px"
        >
          <path d="M 12 2 C 10.343 2 9 3.343 9 5 L 9 11 C 9 12.657 10.343 14 12 14 C 13.657 14 15 12.657 15 11 L 15 5 C 15 3.343 13.657 2 12 2 z M 6.0878906 11 C 5.4818906 11 4.9937969 11.537719 5.0917969 12.136719 C 5.5816755 15.136436 7.9811339 17.488992 11 17.921875 L 11 20 C 11 20.552 11.448 21 12 21 C 12.552 21 13 20.552 13 20 L 13 17.921875 C 16.018866 17.488992 18.418325 15.136436 18.908203 12.136719 C 19.006203 11.537719 18.518109 11 17.912109 11 C 17.418109 11 17.010734 11.363563 16.927734 11.851562 C 16.522734 14.206563 14.471 16 12 16 C 9.529 16 7.4772656 14.206563 7.0722656 11.851562 C 6.9892656 11.363563 6.5828906 11 6.0878906 11 z"></path>
        </svg>
      </button>
      <button
        className="btn btn-light rounded-circle p-3 shadow"
        onClick={handleStopListening}
        disabled={!isListening}
      >
        {/* Stop Icon */}
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="48px"
          height="48px"
          viewBox="0 0 24 24"
        >
          <path d="M6 6h12v12H6z" />
        </svg>
      </button>
      <div className="helper-text mt-2">
        {isListening ? 'Listening...' : 'Click to activate'}
      </div>
      <div className="transcript mt-2">
        <strong>Transcript:</strong> {transcript}
      </div>
      {keywordDetected && (
        <div className="alert alert-success mt-2" role="alert">
          Keyword detected!
        </div>
      )}
    </div>
  );
};

export default KeywordDetection;
